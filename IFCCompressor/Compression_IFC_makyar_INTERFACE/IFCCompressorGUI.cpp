...
namespace IFCCompressorGUI {
	...
	String^ MainForm::GetIFCCompressorPath()
	{
		try
		{
			...
			String^ compressorPath = Path::Combine(parentPath, "\\IFCCompressor.exe");
			...
			compressorPath = Path::Combine(parentPath, "IFCCompressor.exe");
			if (File::Exists(compressorPath))
			{
				return compressorPath;
			}
			
			compressorPath = Path::Combine(parentPath, "Release\\IFCCompressor.exe");
			if (File::Exists(compressorPath))
			{
				return compressorPath;
			}
