...
const int IFCCompressor::MAX_PRECISION = 6;
const double IFCCompressor::DEFAULT_FPR = 0.01;
bool IFCCompressor::FPR = false;
int IFCCompressor::MAX_PRECISION_VAL = 2;
...
bool IFCCompressor::CompressFile(const string& inputFile, const string& outputFile, 
    ...
    FPR = (fprValue != 0.01);  // Active FPR si différent de la valeur par défaut
    MAX_PRECISION_VAL = static_cast<int>(-log10(fprValue));
    
    cout << "Début de la compression..." << endl;
    cout << "Chargement du fichier IFC..." << endl;
    
    ifstream ifile(inputFile);
    if (!ifile.is_open()) {
        cerr << "Erreur: Impossible d'ouvrir le fichier d'entrée: " << inputFile << endl;
        return false;
    }
    ...
    int newInc = 0;              // Nouveau numéro d'instance
    int iterCount = 0;           // Compteur d'instances traitées par itération
    int deleteCount = 0;         // Compteur d'instances supprimées
    ...
    // **ÉTAPE 1: Traitement des nœuds terminaux**
    vector<int> IfcCite;
    IfcInstance* ifcIst;
    ...
    for (getline(ifile, ifcInstance); ifcInstance != "ENDSEC;"; getline(ifile, ifcInstance)) {
        ...
        if (totalCount % 50000 == 0) {
            cout << "Lignes traitées: " << totalCount << endl;
        }
        
        IfcCite.clear();
        ifcIst = new IfcInstance();
        ifcIst->id = IfcInstanceAna(ifcInstance, ifcIst->type, ifcIst->content, IfcCite);
        ifcIst->flag = IfcCite.empty();  // Nœud terminal si pas de citations
        
        if (ifcIst->flag) {
            // Instance terminale - traitement immédiat
            ++iterCount;
            ...
            if (type_iter == mSet.type_content.end()) {
                ...
                if (content_iter == type_iter->second.end()) {
                    ...
                    // Contenu dupliqué - suppression
                    ++deleteCount;
                    mSet.id_baseID[ifcIst->id] = content_iter->second;
                }
            }
        } else {
            // Instance avec références - ajout à UnmatchSet
    ...
    cout << "Fichier chargé avec succès. " << totalCount << " instances trouvées." << endl;
    cout << "Nœuds terminaux identifiés: " << iterCount << endl;
    cout << "Step 1:" << iterCount << " delete:" << deleteCount << endl;
    
    // **ÉTAPE 2: Traitement itératif des instances avec références**
    int iterationTime = 0;
    
    while (iterCount > 0) {
        ++iterationTime;
        cout << "Itération " << iterationTime << "..." << endl;
        ...
        for (size_t i = 0; i < umSet.instance.size(); ++i) {
            if (!umSet.flag[i]) continue;  // Instance déjà traitée
            
            // Vérifier si toutes les références peuvent être résolues
            ...
            if (UpdateReference(tempContent, mSet.id_baseID)) {
                ...
                --umSet.totalCount;
                umSet.flag[i] = false;  // Marquer comme traité
            }
        }
        ...
        if (iterationTime > 20) {
            cout << "Arrêt: Nombre maximum d'itérations atteint." << endl;
            break;
        }
    }
    ...
    // **ÉTAPE 3: Écriture du fichier compressé**
    ofstream ofile(outputFile);
    if (!ofile.is_open()) {
        cerr << "Erreur: Impossible de créer le fichier de sortie: " << outputFile << endl;
        return false;
    }
    
    ofile << ifcHead;
    
    // Écrire les instances compressées
    ...
    // Génération du rapport CSV si demandé
    if (generateCSV) {
